...
| 2026-06-13 | **Pass 1 分段逻辑升级** | 引入“片段切割专家”专属提示词，增加 `unanalyzable` 逃生通道；在后端 `validateSegments` 中引入“首尾缓冲（±1秒）”和“智能合并（≤5秒）”算法，解决片段碎片化与关键动作被截断的问题。 |
| 2026-06-16 | **Pass 1.5 引入音频转写 CoT** | 在 Pass 1.5 JSON 输出中强制最优先输出 `audio_transcript`（音频逐字转写），迫使模型在分析动作前进行“听写”；并增加对短促教练指令（如“蹲”、“起”）的敏感度提示词，以解决高频短促音频在线索推理中被视觉特征淹没的问题。 |
| 2026-06-20 | **防并发处理与语音识别鲁棒性升级** | 1. 修复由于多进程环境导致的 `analysis_jobs` 并发抢占问题，通过 `UPDATE ... AND status = 'queued'` 实现原子级排他锁。<br>2. Pass 1 缓冲时长向前扩充至 3 秒，确保完整捕获击球前教练指令。<br>3. 在 Pass 1.5 增加抗底噪混淆提示词。<br>4. **修正**：优化抗混淆提示词，防止过度拟合（如将“对”强行听成“蹲”），引入“视听交叉验证”机制，要求结合实际视觉画面（有无真实下蹲）消解同音字歧义。 |
...
优化后的架构设计（基于精简 Prompt + 强大后端兜底）：

1. **精准角色定位与约束 (Prompt)**：
   - 设定大模型为“片段切割专家”，专注时间切割而非动作分析。
   - 明确过滤“纯讲解/示范（无击球）”等边缘无效场景。
   - 强制收敛 `description` 为有限枚举（如多球训练、发球练习等），严禁使用具体技术名词。
   - 针对完全不可用的视频，支持返回 `unanalyzable` 字段与原因。

2. **后端智能分段与缓冲 (Code)**：
   - 依赖 `validateSegments()` 进行二次校验。
   - **安全缓冲**：将模型识别的起止时间自动向前延长 1 秒、向后延长 1 秒，抵消低帧率（2fps）造成的截断误差，完整保留引拍与随挥。
   - **智能合并**：对缓冲后的片段，若相邻时间差 ≤ 5 秒，则由代码自动融合为一个连续片段。
...
## 18. 视听融合多模态分析与分层推理 (Pass 1.5 优化)

在基础帧率优化（看清动作）的基础上，我们在 Pass 1.5（分类阶段）引入了原生多模态能力与严格的分层推理逻辑（CoT）。

### 18.1 核心机制

1. **听觉纠偏**：大模型被明确要求倾听视频音轨中的教练/同伴口令（如"下蹲"、"转腰"、"拉球"）。如果语音有【高置信度】的明确技术指向，它将覆盖视觉判断（如动作幅度偏小导致视觉上像拨球时，依据语音纠正为拉球）。
2. **三级方位置信度**：引入【正手/反手】的置信度判定。特别是针对"侧身正手"等边界情况，避免因为球员站在左半台而误判为反手。只有在高置信度方位下，才会触发严格的 `fh_` 或 `bh_` 选项隔离规则。
3. **结构化思考过程**：在 `analysis_notes` 字段中强制输出结构化的 6 步推导结论，极大提升后期的溯源和监控效率。

### 18.2 提示词核心伪代码

```text
第1步：聆听语音指令，评估语音置信度（高/中/低）
第2步：判断击球方位，评估方位置信度（高/中/低），高置信度方位触发强隔离规则
第3步：综合研判技术类型（结合视觉幅度和语音信号）
第4步：输出结构化的 analysis_notes
```

### 18.3 配置变更

将 `techniqueClassifier.ts` 中与 Gemini 交互的 `temperature` 设定维持在 `0.2`。
配合分层推理 Prompt，`0.2` 在保证严格推理链路不偏离轨道的同时，为模型保留了评估置信度的基础空间。
