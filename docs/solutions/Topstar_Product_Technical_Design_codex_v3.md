...
async function runHealthCheck(tutorials: Tutorial[]) {
  ...
  for (const t of tutorials) {
    ...
    const consecutiveFailures = probe === 'suspect'
      ? (t.consecutive_failures ?? 0) + 1
      : 0;

    await db.tutorials.update(t.tutorial_id, {
      status: probe === 'dead_confirmed' ? 'dead' : probe,
...
**状态流转建议**：
- 自动探测只负责把条目标成 `active` 或 `suspect`
- 明确 404/410、人工确认、或多源证据一致时再标成 `dead`
...
**目标**：消灭 `setTimeout` 假流程，建立真实的任务状态机。报告内容仍为模板生成，但 job 本身必须是持久化、可轮询、可重试的。
...
`enqueueAnalysisJob()` 在 MVP 可以有两种实现：
