...
### 18.4 视频帧率优化与多模态听觉分析 (v1.11 新增)

在真实的视频分析流水线（阶段 B）中，为了解决大模型因帧率过低而"看不清"，以及单纯依赖视觉导致"容易被初学者变形动作误导"的痛点，我们在 Pass 1.5（技术动作分类）引入了原生多模态能力与严格的分层推理逻辑。

1. **自定义高采样率**：通过 `videoMetadata: { fps: 5.0 }` 参数将 API 采样率提升至 5fps，精准捕捉 0.01 秒级的核心帧（如球拍触球瞬间）。
2. **听觉指令纠偏**：要求大模型倾听视频中的教练/同伴语音口令，将其分为高/中/低三级置信度。若语音指令具有高技术指向置信度（如明确喊出"拉球"、"摩擦"），可覆盖视觉判断，修正因动作过小导致的技术类型误判。
3. **分层置信度隔离**：构建"评估语音置信度 -> 评估方位置信度 -> 综合定性"的强制推理路径。只有当方位置信度为"高"时，才触发强制的左/右手柄选项隔离，有效处理"侧身正手"等复杂边界场景。
4. **稳定度控制**：将分类步骤的模型 `temperature` 维持在 `0.2`，在约束大模型分层推理的同时保留部分不确定性空间。同时要求大模型输出结构化的 `analysis_notes`（包含语音/方位判断中间结果），以支持下游流程监控。
...
### 18.8 Pass 1 智能分段切割算法 (Smart Segmentation Algorithm)

为了解决低帧率（2fps）采样导致的动作截断问题以及短暂停顿导致的片段碎片化问题，对 Pass 1 切割算法进行“精简Prompt + 强数学兜底”的重构：

#### 18.8.1 Prompt 约束级优化
- **角色定位**：重塑为纯粹的“片段切割专家”，专注时间切割，禁止技术猜想。
- **边界定义**：严格过滤无击球的纯讲解示范、慢动作回放等边缘无效场景。
- **强制收敛**：引入封闭的 `description` 选项集，严禁模型输出技术动作名词，彻底根绝先入为主的技术锚定。
- **逃生通道**：新增 `unanalyzable` 字段与原因输出，针对纯黑屏、极端模糊等劣质视频提供优雅的错误回传，取代僵硬的格式解析报错。

#### 18.8.2 后端兜底与缓冲合并
在 `validateSegments` 中实装基于数学计算的二次重组算法：
- **安全缓冲期（Padding）**：强制将每个片段的起点向前拓宽 1 秒、终点向后拓宽 1 秒。通过空间换时间，弥补 2fps 采样的时间戳精度损失，百分百抓取发力的前置引拍（Backswing）与击球后的随挥还原（Follow-through），彻底解决“动作看不全”导致的误判。
- **极短停顿合并（Merging）**：将经过缓冲后的片段按时间线排序，凡是时间间隔 ≤ 5 秒的独立片段，由代码自动合并为单一长片段。以此消除换球、擦汗等微小停顿造成的碎片化开销，向 Pass 1.5 提供连贯的组合动作分析上下文。
