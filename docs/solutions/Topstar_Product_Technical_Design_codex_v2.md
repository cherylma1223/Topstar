...
教程库中 5000+ 条抖音/B 站外链，平台方随时可能下架或改变 URL 结构，`status` 和 `last_verified_at` 字段如果没有自动 enforcement，会迅速腐化成无效数据。用户侧的表现是：点击推荐链接 -> 404/空白页，信任感大幅受损。

### 17.2 解决方案：三层可用性治理

#### 层 1：定期探测 Job（离线）

在服务端新增一个定期运行的可用性检查脚本 `server/jobs/link-health-check.ts`，对所有 `status !== 'dead'` 的教程条目做 HTTP HEAD 请求，更新状态：
...
async function checkOne(tutorial: Tutorial): Promise<'active' | 'dead' | 'suspect'> {
  try {
    ...
    return 'suspect';   // 5xx / 403 / 其他 -> 暂标疑似
  } catch {
    return 'suspect';   // 超时 / 网络错误 -> 先标疑似，连续 3 次再 dead
  }
}
...
async function runHealthCheck(tutorials: Tutorial[]) {
  ...
  for (const t of tutorials) {
    const lastCheck = t.last_verified_at ? new Date(t.last_verified_at).getTime() : 0;
    ...
    const newStatus = await checkOne(t);
    await db.tutorials.update(t.tutorial_id, {
      status: newStatus,
      last_verified_at: new Date().toISOString(),
      // 连续失败计数：suspect 累计 >= 3 次 -> dead
      consecutive_failures: newStatus === 'suspect'
        ? (t.consecutive_failures ?? 0) + 1
        : 0,
    });
    if ((t.consecutive_failures ?? 0) + 1 >= 3 && newStatus === 'suspect') {
      await db.tutorials.update(t.tutorial_id, { status: 'dead' });
    }
  }
}
```
...
编排层 `recommendTutorials()` 必须过滤 `status === 'dead'` 的条目，并在结果不足时触发降级：
...
export async function recommendTutorials(
  ...
  const candidates = await db.tutorials.findMany({
    where: {
      status: { in: ['active', 'suspect'] },   // suspect 暂时保留，但降权
      ...(actionId ? { related_action_ids: { has: actionId } } : {}),
    },
    orderBy: [
      { status: 'asc' },          // active 排在 suspect 前面
      { quality_score: 'desc' },
      { last_verified_at: 'desc' },
    ],
    take: limit * 3,              // 多取，留 fallback 余量
  });
  ...
  // 降级：若全部为 suspect，在 answerText 中插入提示
...
`last_verified_at` 语义保持"人工最后确认时间"，`last_checked_at` 为系统探测时间，两者分开，便于区分人工与自动数据来源。
...
**目标**：消灭 `setTimeout` 假流程，建立真实的任务状态机，报告内容仍为模板生成，但流程可观测、可重试。
...
// 1. 用户上传视频 -> 保存文件 -> 创建 job 记录
async function createAnalysisJob(videoFile: File, userId: string): Promise<Job> {
  const jobId = uuid();
  const filePath = await saveToStorage(videoFile, `analysis/${jobId}`);
  await db.analysisJobs.create({
    id: jobId, userId, status: 'queued',
    video_path: filePath, created_at: new Date()
  });
  // 异步触发处理，立即返回 job_id
  setImmediate(() => processJobStageA(jobId));
  return { job_id: jobId, status: 'queued' };
}

// 2. 阶段 A：用视频文件名 + 用户自述文字生成模板报告
async function processJobStageA(jobId: string) {
  await db.analysisJobs.update(jobId, { status: 'running' });
  try {
    // 调用文本模型，输入：用户填写的"描述自己的问题"字段
    const userDesc = await db.analysisJobs.getUserDesc(jobId);
...
**用户体验说明**：阶段 A 的报告基于用户文字描述生成，非真实视频理解。前端需在报告卡片顶部加一行小字说明："本报告基于您的描述生成，视频帧分析功能即将上线"，而不是假装已经看了视频。
...
| 项目 | 参数 |
|------|------|
| Gemini 1.5 Pro 视频输入 | 约 $0.00265 / 秒视频（截至 2026-Q1） |
| 最大视频时长 | 单次最大约 1 小时（Files API 限制） |
| 处理延迟 | 上传 + 处理约 1-5 分钟（视频越短越快） |
| 建议用户侧限制 | MVP 限制上传视频 ≤ 3 分钟（控制成本 + 等待时间） |

**阶段 B 切换方式**：只需将 job 处理函数从 `processJobStageA` 切换为 `processJobStageB`，对外 API 接口完全不变，前端无需改动。
...
教程库中 5621 条数据里，绝大多数 `quality_score: 0`（未标注）。文档设计的推荐排序"quality_score 优先"在这种情况下实际退化成随机排序，无法保证用户拿到的是质量较好的教程。
...
interface ScoringSignals {
  ...
  platform: 'bilibili' | 'douyin';   // B 站教学内容质量相对更稳定
...
function computeInitialScore(t: Tutorial): number {
  ...
  // 信源质量
  if (t.author)       score += 1;
  if (t.platform === 'bilibili') score += 0.5;  // B 站教学视频整体质量稍高
...
这样即使全部都是"自动打分"，推荐时优先级也是**有动作关联且有 author 的 B 站视频 > 无关联的泛教程 > hashtag 堆砌的无效内容**，比全部 0 分随机排序有显著改善。
...
```typescript
// 点击率高于平均值 1 倍以上 -> score += 0.5（每次最多 +0.5，上限 5.0）
// 持续 30 天无点击且被展示过 -> score -= 0.3
```
...
- **Phase 1 前置**：运行 `bootstrap_quality_scores.ts` 脚本，为现有 5621 条记录生成初始评分（预计几分钟内完成）
- **Phase 1 并行**：`recommendTutorials()` 的排序逻辑改为按 `quality_score DESC`，而不是随机/默认顺序
- **Phase 2**：前端加教程点击上报，服务端写入 `usage_events`
