...
更新日期：2026-03-22  
状态：可落地（MVP -> 可演进生产级）  
范围：知识库系统 + 意图识别 + 知识编排层 + 回复模板（结构化输出）+ 教程推荐 + 视频分析任务化 + 数据持久化 + 外链可用性治理 + 视频分析实现路径 + 推荐质量冷启动 + 统一化专家建议卡片 UI
...
- **协议优先于文案**：前端应依赖服务端定义的锁定结构或占位符渲染锁区，不应依赖模型是否“恰好”按模板生成了独立的 VIP 标题块。
...
#### 8.2.3 交互增强
- **TTS 同步**：点击朗读按钮时，需按顺序朗读总结文字 -> 动作要领。
- **视频跳转**：卡片内的视频条目保持“一键直达”嵌入式播放器的交互。
- **渐进展示不得改变内容层级**：若前端使用打字机效果或分阶段 reveal，动画状态机只能影响“何时显示”，不得改变既定内容层级与最终显示集合。尤其在 `AnalysisReportCard` 中，即使 `problems` / `improvements` 为空，只要 `summaryText`、`videoLinks` 或 VIP Block 存在，`summaryText` 展示完成后仍必须继续显示视频教程区与 VIP 区块；不得因为阶段推进逻辑绑定在标题、空数组分支或中间步骤缺失而导致底部模块被永久跳过。
- **阶段推进责任需单点收口**：同一卡片中，负责推进 reveal 阶段的回调必须有唯一主控来源。若标题与总结同属首阶段，禁止由标题完成回调提前推进到下一阶段，导致总结、视频教程或 VIP Block 的后续显隐条件失效。

#### 8.2.4 Frontend VIP Content Rendering & UX 指引

前端在处理 `【核心秘诀】` 或 `【VIP专属】` 区块时，除基本的视觉模糊外，需遵循以下交互逻辑：

前提约束：前端只消费服务端已经脱敏后的结果。若后端误把 VIP 明文下发到普通段落，前端不得被视为可接受的安全补救手段。

1. **诱惑性占位（Temptation UX）**：
   - 即便后端返回的是脱敏后的占位符或提示语，前端仍必须应用 **毛玻璃（Blur）+ 锁头图标** 的组合视觉效果。
   - 保持一定的展示区域高度，暗示内容的丰富度，激发用户点击/升级的好奇心。
   - 该区域应复用既定的锁卡视觉语言（已确认的 mockup 风格），包括毛玻璃背景、锁头图标、CTA 按钮和整体气质；禁止为了“先跑通功能”临时替换成与现有体验不一致的新卡片样式。
2. **转化闭环（Conversion CTA）**：
   - 锁定区域应作为一个整体的交互点。点击该区域时，需弹出或跳转至“会员升级/账户充值”页面。
   - 建议在模糊层上方叠加醒目的“立即解锁”按钮（CTA 样式）。
3. **内容防盗增强（Frontend Protection）**：
   - **禁止复制**：对该区域应用 `user-select: none`，并拦截右键/长按操作。
   - **语音静默**：TTS 朗读逻辑必须物理跳过该 Block 的渲染文本。
4. **状态联动**：
   - 当 `App` 层捕获到用户身份从 `guest` 升级为 `vip` 时，应立即更新消息列表状态并“秒级”触发解除模糊动画（Un-blur transition），提供即时的付费反馈感。
5. **渲染判定规则**：
   - 锁区渲染应优先基于服务端协议字段、固定标题结构或系统占位符触发，而不是通过正文关键词猜测“这句话像不像 VIP 内容”。
   - 若发现普通段落中仍残留疑似 VIP 明文，前端可做防御性隐藏或替换，但该逻辑仅作为回归兜底，不可替代服务端脱敏。
   - 当收到 `[LOCKED_VIP_CONTENT]` 这类系统占位符时，组件必须在文本清洗后仍能正确识别锁定态，不能因 Markdown/样式清洗把占位符破坏成普通字符串，更不能把该 token 原样渲染到界面上。

#### 8.2.5 TTS 播放链路与性能规范

当前产品中的 TTS 不仅是“能播出来”，还必须满足移动端即时反馈体验。文档在本节补充以下强约束，避免实现退化为“点击后同步生成整段音频，再等待很久才开始播放”。

1. **责任边界必须收口到服务端**：
   - TTS 预读取文本的生成、文本清洗、VIP 剔除、分段策略、缓存 key 生成，必须由服务端统一负责。
   - 前端不得再通过关键词匹配、正则猜测或组件私有逻辑自行裁剪 TTS 文本；前端仅消费服务端返回的安全文本或分段播放计划。
   - 同一条回答在聊天气泡、诊断卡片、未来通知朗读等不同入口，必须复用同一套服务端 TTS 脱敏规则，避免链路不一致。

2. **默认采用“预生成优先”，禁止把整条等待压到点击时刻**：
   - 当 `answerText` 或 `AnalysisReport` 完成渲染后，前端应在后台静默请求 TTS 预生成或预取首段音频。
   - 用户点击朗读按钮时，默认行为应是“立即播放已准备好的首段”，而不是“此时才开始调用模型合成整段音频”。
   - 若设备、网络或节流策略不允许全量预生成，也至少要保证首段（summary）可被优先预取。

3. **朗读内容必须按语义分段，不得把整张卡片一次性拼成长文本**：
   - 自由问答气泡：优先按“导语/总结”与“正文要点”拆段。
   - `AnalysisReportCard`：必须按“总结文字 -> 技术问题/技术特点 -> 训练建议/战术指导”的顺序拆段，与交互文案保持一致。
   - 视频标题、URL、CTA 文案、锁定占位符、VIP Block、升级按钮文案，均不得进入 TTS 输入。
   - 分段策略应以服务端结构化字段为准，不得依赖前端把多个字段临时 `join('。')` 后再交给 TTS。

4. **首播策略必须面向“尽快出声”优化**：
   - 允许首段音频生成完成后立即开始播放，后续段可并行拉取、顺序拼接播放。
   - 禁止要求“所有段落全部生成完成并全部解码完毕后”才允许开始播放。
   - 若底层模型或网关暂不支持真正的流式音频输出，则在产品层通过“文本分段 + 首段优先”达到近似流式起播体验。

5. **缓存策略必须跨组件，而不是仅存在于单个 React 实例内**：
   - 缓存 key 应由服务端根据 `voice + sanitizedText + userTier + locale + appVersion` 等稳定字段生成。
   - 前端至少应具备会话级缓存；同一条消息在重新渲染、折叠展开、页面切换后，不应重复发起整段 TTS 合成。
   - 服务端可进一步增加短期缓存，减少重复点击导致的上游模型调用。

6. **重试与降级必须克制，避免放大等待时间**：
   - TTS 属于强交互链路，前后端不得同时做长指数退避重试，避免用户一次点击被累计放大为数秒级等待。
   - 推荐由服务端承担有限次数的短重试；前端仅做一次轻量重放或直接展示“语音暂不可用，请重试”。
   - 当首段生成失败时，前端应立即结束 loading 并恢复按钮状态，不得出现长时间无反馈转圈。

7. **需要显式性能指标，作为验收门槛**：
   - `Time To First Audio`（TTFA，点击朗读到用户听到第一帧语音的耗时）应作为核心体验指标。
   - 目标建议：`P50 < 1.5s`，`P95 < 3.0s`；若超出，应优先优化分段、预生成与缓存，而不是继续堆叠重试。
   - 需记录日志字段：`request_id`、`message_id`、`cache_hit`、`segment_count`、`ttfa_ms`、`total_tts_ms`、`provider_model`，便于后续回归分析。
...
1. `POST /api/v2/chat`
   ...
   - `prefs` 至少应包含用户权限信息（如 `is_vip`）；若后续统一为 `UserContext.tier`，需保持语义等价
   - 输出：`ChatResponse`
2. `POST /api/v2/tts`
   - 输入：`message_id?`, `answerText?`, `report?`, `prefs`, `voice?`, `locale?`
   - 服务端职责：完成 TTS 文本清洗、VIP 剔除、分段、缓存命中判定
   - 输出：`TTSResponse`
   - 兼容策略：`v1` 可继续保留为简单代理接口，但新的前端实现应优先迁移到 `v2`
3. `POST /api/v2/tutorials/recommend`
   - 输入：`query`, `action_id?`, `limit?`
   - 输出：`tutorialVideos[]`
4. `POST /api/v2/analysis/jobs`
   - 创建分析任务：返回 `job_id`
5. `PUT /api/v2/analysis/jobs/{job_id}/upload`
   - 上传或提交 `video_url`
6. `GET /api/v2/analysis/jobs/{job_id}`
   - 查询状态：`queued|running|done|failed` + `report`

### 9.1 TTSResponse（建议）

`/api/v2/tts` 不应只返回一整段 base64 音频字符串，而应返回可支持“首段优先播放”的结构化协议：
...
协议约束：
- `segments` 内的 `text` 必须已经过服务端清洗，可直接用于调试和埋点，不得包含 VIP 明文或锁定占位符。
- 若未来支持真正的流式音频网关，可保持 `segments` 语义不变，仅替换 `audio` 的载荷形态，不破坏前端播放逻辑。
- 前端应允许在拿到 `order = 0` 的首段后立即开始播放，同时后台准备后续段。

### 9.2 统一错误响应规范（`ErrorResponse`）
...
| `TTS_GENERATION_FAILED` | 503 | TTS 生成失败 | true |
| `TTS_TEXT_INVALID` | 400 | TTS 输入为空、不可朗读或未通过清洗校验 | false |
